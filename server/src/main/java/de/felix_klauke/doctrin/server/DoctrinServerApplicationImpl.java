...
public class DoctrinServerApplicationImpl implements DoctrinServerApplication {
    ...
    public void handleNewConnection(DoctrinServerConnection connection) {
        ...
        connection.getMessages().subscribe(doctrinMessageWrapper -> coreApplication.handleMessage(doctrinMessageWrapper.getMessageContext(), doctrinMessageWrapper.getMessage()), throwable -> {

        }, () -> {
            coreApplication.handleInactiveSubscriber(connection.getRemoteName().blockingFirst());
        });
    }
}
